...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  if (!parsed.success) {
    return err('Flight selection request is invalid.', 'VALIDATION_ERROR', 400, parsed.error.flatten())
  }

  try {
    const selection = await new TripOfferSelectionService().selectFlight({
      tripId,
      userId: guard.userId,
    ...
    return NextResponse.json({ flightSelection: selection })
  } catch (error) {
