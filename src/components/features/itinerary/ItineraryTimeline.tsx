...
export function ItineraryTimeline({ roadmap }: ItineraryTimelineProps) {
  ...
  return (
    <section className="surface-panel p-card-pad">
      <div>
        <p className="text-sm font-semibold uppercase tracking-wide text-atlas-700">Roadmap</p>
            ...
            <ol className="mt-3 space-y-3 border-l border-neutral-200 pl-4">
              {day.items.map((item, index) => (
                <li key={`${day.dayNumber}-${item.label}-${index}`} className="relative">
                  <span className="absolute -left-[1.62rem] flex h-5 w-5 items-center justify-center rounded-full bg-neutral-900 text-[10px] font-bold text-white">
